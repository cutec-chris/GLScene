...
uses GLKeyboard, OpenGL1x, GLContext, GLState, GLTextureFormat,GL;
...
begin
   SetCurrentDirUTF8(ExtractFilePath(Application.ExeName)+DirectorySeparator+'Data'); { *Konvertiert von SetCurrentDir* }
   ...
   for i:=0 to 3 do for j:=0 to 3 do begin
      ...
      if not FileExistsUTF8(name) { *Konvertiert von FileExists* } then begin
...
begin
   ...
   with GLCamera.Position do begin
   ...
   with DCCamera.Position do begin
...
begin
   case Key of
      ...
      '*' : with TerrainRenderer do
         if CLODPrecision>1 then CLODPrecision:=Round(CLODPrecision*0.8);
      '/' : with TerrainRenderer do
         if CLODPrecision<1000 then CLODPrecision:=Round(CLODPrecision*1.2+1);
   end;
   Key:=#0;
end;
...
const
   ...
   begin
      px:=x+rx+s2;
      py:=y+ry+s2;
      if hd.DataState=hdsNone then begin
         alpha:=1;
      end else begin
...
var
   ...
   sbp, sbr : TVector;
begin
   if WakeVertices=nil then begin
   ...
   // Progress wake
   if newTime>DOWake.TagFloat then begin
      if DOWake.TagFloat=0 then begin
      ...
      if WakeVertices.Count>=80 then begin
...
var
   ...
   c : Single;
begin
   if not Assigned(WakeVertices) then Exit;
   if (not FFSailBoat.Visible) or (not WaterPlane) then Exit;
