{ $Id: testlpi.pas 19313 2009-04-10 15:33:23Z paul $}
...
begin
  //assume lazbuild is in the env path
  LazarusDir    := 'c:\lazarus\';
  ComponentsDir := SetDirSeparators('../Lazarus/');
  ExamplesDir := SetDirSeparators('../Demos/');
...
class function TLpkTest.CreateSuiteFromDirectory(
  const AName, ABasePath: string): TTestSuite;
...
begin
  ...
  try
    ...
    writeln(LazBuild.CurrentDirectory);
    LazBuild.Execute;
    OutputLines := ReadOutput(LazBuild);
    if LazBuild.Running then
    begin
...
begin
  ...
  ATestSuite.AddTest(TLpkTest.CreateSuiteFromDirectory('Components', ComponentsDir));
  GetTestRegistry.AddTest(ATestSuite);

  // Create testsuite for demo files
  ATestSuite := TTestSuite.Create('Demo files');
  ATestSuite.AddTest(TLpiTest.CreateSuiteFromDirectory('Compilation', ExamplesDir));
  GetTestRegistry.AddTest(ATestSuite);
end;
